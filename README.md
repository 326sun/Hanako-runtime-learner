...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-851%2F851-success" alt="tests">
</p>
...
v5.0.0 是现代化发布基线：M0 引入自包含 dist/zip 发布包，M2 增加默认关闭的 LLM extraction worker，M3-lite 将后台整理迁移到 Hanako `task:*` bus 调度，M6 完成版本、治理和 release check 收口。本版本**不包含** M1 本地 embedding / vector index、M4 Agent 编排或 M5 adaptive thresholds。
...
```powershell
git clone --branch v5.1.3 https://github.com/326sun/Hanako-runtime-learner.git
cd Hanako-runtime-learner
npm install
npm run build
npm run install-plugin
...
npm test: 851 tests, 846 passed, 5 skipped
