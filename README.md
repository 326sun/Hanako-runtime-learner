...
v5.0.0 是现代化发布基线：M0 引入自包含 dist/zip 发布包，M2 增加默认关闭的 LLM extraction worker，M3-lite 将后台整理迁移到 Hanako `task:*` bus 调度，M6 完成版本、治理和 release check 收口。v5.1.5 已补齐一部分可选/受治理路径，并完成入口/控制面结构收敛；默认边界仍保持保守：
...
```powershell
git clone --branch v5.1.5 https://github.com/326sun/Hanako-runtime-learner.git
cd Hanako-runtime-learner
npm install
npm run build
npm run install-plugin
...
| [docs/ACCEPTANCE-v5.1.5.md](docs/ACCEPTANCE-v5.1.5.md) | 当前版本验收记录。 |
