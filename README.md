...
<p align="center">
  <img src="https://img.shields.io/badge/version-5.1.8-blue" alt="version">
...
v5.0.0 是现代化发布基线：M0 引入自包含 dist/zip 发布包，M2 增加默认关闭的 LLM extraction worker，M3-lite 将后台整理迁移到 Hanako `task:*` bus 调度，M6 完成版本、治理和 release check 收口。v5.1.7 在 v5.1.6 基础上完成子系统级精简计划：`tools/control.js` 从控制面聚合器收敛为路由器（533→268 LOC，32→18 imports），新增防回流结构规则与自动化 drift 检测。v5.1.8 是干净补丁发布：usage pattern 会写入结构化 evidence，doctor 的 `evidence_missing` 不再把 rejected pattern 算作健康噪音；默认边界与所有安全/审批/事务/审计闸门保持不变：
...
```powershell
git clone --branch v5.1.8 https://github.com/326sun/Hanako-runtime-learner.git
cd Hanako-runtime-learner
npm install
npm run build
npm run install-plugin
...
| [docs/ACCEPTANCE-v5.1.8.md](docs/ACCEPTANCE-v5.1.8.md) | 当前版本验收记录。 |
