...
**四层管道**：事件捕获 → 模式检测 → 记忆管理与衰减 → 检索与治理。完整调用拓扑见 [`ARCHITECTURE.md`](ARCHITECTURE.md)。

核心设计决策：
- **零运行时依赖**：纯 JS BM25 倒排索引，CJK 单字+二元组分词，无需 SQLite 或分词器
...
| 键 | 默认 | 说明 |
|---|---|---|
| `governanceProfile` | `balanced` | 策略配置档：`conservative` / `balanced` / `autonomous` |
...
| `includePendingPreferences` | `false` | 未审核偏好是否注入。默认关闭：未审核纠正仅可检索 |
...
| 键 | 默认 | 说明 |
|---|---|---|
| `modelAdvisorEnabled` | `false` | 启用后台整理（开启后会外发归纳后的 workflow/error/usage 模式） |
...
| 键 | 默认 | 说明 |
|---|---|---|
| `semanticSearchEnabled` | `false` | 启用 RRF 语义融合（开启后会外发查询词与候选文本到 embedding 端点） |
...
- **CJK 分词**：单字 + 相邻二元组，`排版` 可命中 `论文排版`，无需外部分词器
