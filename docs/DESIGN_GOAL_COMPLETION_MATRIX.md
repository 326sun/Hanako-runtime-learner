...
当前版本：`5.1.7`（在 v5.0 冻结表面上追加默认关闭/只读的实验能力，外加 model advisor 可观测性补强、v5.1.1 全面审计加固、v5.1.3 的 profile 安全闸门修复与设置面板即时生效、v5.1.4 的性能/复杂度治理收口、v5.1.5 的入口/控制面结构收敛、v5.1.6 的文档统计修正，以及 v5.1.7 的子系统级精简计划——`tools/control.js` 从控制面聚合器收敛为路由器，新增防回流结构规则与自动化 drift 检测，默认边界与安全闸门不变）
