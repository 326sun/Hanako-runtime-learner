...
> **`5.1.0`（内部安装候选，未发布）**：在冻结的 v5.0 API 表面之上，仅追加默认关闭/只读的实验能力（M5d adaptive-threshold 建议、M4 只读 agent graph 及 `agent_graph_preview` 诊断入口、M5/M5b feedback 信号）。不改动、不移除任何已冻结的接口、安全边界或人工确认规则。
