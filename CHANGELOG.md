...
- **Performance (defensive): batched pattern eviction**. `PatternDetector.pruneMemory()` now collects all ids to evict across its three stages and forgets them in a single `_forgetPatterns()` pass, so the orphan-relation cleanup runs once (O(n+k)) instead of once per pruned pattern (O(k·n)). At the bounded operating size (store capped at `MAX_PATTERN_COUNT*2 = 100`) this is invisible (≈0.07 ms either way); the win is worst-case behavior if the store ever grows abnormally — measured pruning at N=2000 dropped from ≈11 ms to ≈2 ms. Behavior is identical to the previous per-id path (golden-equivalence test added). Mirrors the v4.2.0 `O(n log n) → O(n)` eviction work.
- **Performance regression guardrail (new)**: `scripts/perf-bench.js` + `benchmarks/perf-thresholds.json` + `npm run perf`. Micro-benchmarks the per-turn hot path (BM25 search, pattern decoration, prune, cached/cold `all()`) at N=50/100 plus cold module-graph import, and fails when any op exceeds a generous advisory ceiling. Advisory only — not wired into the release gate to avoid CI flakiness on slow runners.
- **Measurement: injected context footprint**. Confirmed and pinned (regression test) that the per-turn SKILL.md injection is hard-bounded — a fixed ≈490-token instructional tail plus a budgeted, section-capped (≤5 prefs / ≤3 workflows / ≤3 risks, `maxSkillTokens=800`) dynamic part, ≈954 tokens at saturation — and does not grow with pattern-store size. No change to injection behavior.
- **Measurement finding**: a profiling pass found no runtime CPU bottleneck at the real operating size (every hot-path op is sub-millisecond at the bounded ≤100-pattern store); this release therefore hardens worst-case and locks in a baseline rather than rewriting working hot-path code.
- **Cleanup: removed dead duplicate `lib/rank-fusion.js`**. Its `rankByScore` / `rrfScores` / `fuse` were absorbed into `lib/embeddings.js` (the in-file comment already noted "was rank-fusion.js") and no production code imported the standalone file; its test already imported from `embeddings.js`. Removing it brings the lib module count to 81, matching `ARCHITECTURE.md`. No behavior change.
- **Cleanup: de-duplicated injectability logic**. `decoratePatterns()` no longer inlines a second copy of the inject decision — it delegates to `isInjectable()`, which now accepts an optional precomputed `decayedScore` so the decorate path keeps using its rounded display score (behaviour-preserving; rounding-boundary regression test added). One source of truth for "is this pattern injectable".
- **Cleanup: trimmed dead and over-broad module exports**. Deleted 2 genuinely-unused exported functions (`readJsonlTail` in `jsonl-utils.js`, `registerTransferBatch` in `transfer-registry.js`) and dropped the redundant `export` keyword from 65 helpers that are only used inside their own module — each verified unreferenced across `lib/`, `tools/`, `scripts/`, `tests/`, `index.js`, and `install.cjs`, and absent from the frozen API docs. `lib/hana-runtime-compat.js` was deliberately left untouched (its surface is kept in sync with the sibling project). No behavior change.
- **Final release audit**: README rewritten from scratch for release clarity; release-readiness expected test count synchronized to the current suite; v4.3.2 acceptance evidence refreshed.
- **Performance follow-up**: search/index, prune, skill rendering, config merge, control/stats, and action-feedback hot paths were cleaned up without widening behavior. `npm run perf` now also tracks `skill_render_ms`.
- **Tests**: 515 total (511 passing + 4 Windows symlink skips; was 496 before v4.3.2); benchmark corpus 17/17; perf thresholds pass; release readiness score 100.
- **Safety boundary preserved**: no R4, external side-effect, automation, or security-boundary changes; no public/frozen API or architecture changes. Internal performance and test/benchmark additions only.
