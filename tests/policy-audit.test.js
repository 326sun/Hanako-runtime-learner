...
describe("policy audit", () => {
  it("policy profiles list stable built-in modes", () => {
    const names = listPolicyProfiles().map((p) => p.name).sort();
    assert.deepEqual(names, ["autonomous", "balanced", "conservative"]);
  });

  it("conservative policy enables review-first defaults without enabling external services", () => {
  ...
  it("unknown policy is rejected with available profile names", () => {
    const result = applyPolicyProfile(DEFAULT_CONFIG, "reckless");
    assert.equal(result.ok, false);
    assert.ok(result.available.includes("balanced"));
  });

  it("audit bundle redacts secrets and writes markdown/json files", () => {
