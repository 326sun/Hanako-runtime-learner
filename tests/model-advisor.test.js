...
describe("model advisor", () => {
  ...
  describe("advisorEndpointWarning", () => {
    it("warns for non-local HTTP endpoints only", () => {
      assert.match(advisor.advisorEndpointWarning("http://api.example.com"), /without TLS/);
      assert.equal(advisor.advisorEndpointWarning("http://localhost:11434"), "");
      assert.equal(advisor.advisorEndpointWarning("https://api.example.com"), "");
    });
  });
  ...
  describe("runModelAdvisor", () => {
    ...
    it("keeps usage summary out of advisor prompts unless explicitly enabled", async () => {
      ...
      const usage = { totalRequests: 7, totalTokens: 12345, status: "ok", byModel: { "private-model-name": { requests: 7, totalTokens: 12345 } } };
      const first = await advisor.runModelAdvisor({ config: baseConfig, patterns, usage });
      assert.equal(first.ok, true);
      assert.ok(!sentBody.includes("private-model-name"));
      assert.ok(!sentBody.includes("12345"));

      fs.rmSync(path.join(learnerDir(), "model_advice_state.json"), { force: true });
      sentBody = null;
      const second = await advisor.runModelAdvisor({ config: { ...baseConfig, includeUsageInAdvisorPrompt: true }, patterns, usage });
      assert.equal(second.ok, true);
      assert.ok(sentBody.includes("private-model-name"));
      assert.ok(sentBody.includes("12345"));
    });

    it("records a warning when the private advisor endpoint uses non-local HTTP", async () => {
      globalThis.fetch = async () => jsonResponse();
      const patterns = [{ id: "error:http", type: "error", status: "pending", count: 4, score: 8, desc: "d", fix: "" }];
      const res = await advisor.runModelAdvisor({ config: { ...baseConfig, modelAdvisorBaseUrl: "http://api.example.com" }, patterns });
      assert.equal(res.ok, true);
      assert.match(res.advice.warning, /without TLS/);
    });
