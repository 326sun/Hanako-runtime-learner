...
export function safeAgentTaskId(taskId) {
  return safeFileSlug(taskId, "task");
}

export function agentTasksDir(baseDir) {
  return path.join(baseDir, "agent_tasks");
}

export function agentTaskStatePath(baseDir, taskId) {
  return path.join(agentTasksDir(baseDir), `${safeAgentTaskId(taskId)}.json`);
}

export function summarizeAgentTaskState(state = {}) {
