...
function readSessionFields(value) {
  if (!value || typeof value !== "object") return null;
  const sessionId = value.sessionId || value.id || null;
  const sessionRef = value.sessionRef || value.ref || null;
  const sessionPath = value.sessionPath || value.path || null;
  if (!sessionId && !sessionRef && !sessionPath) return null;
  return { sessionId, sessionRef, sessionPath };
}

export function normalizeSessionTarget(...sources) {
  for (const source of sources) {
    if (!source) continue;
    if (typeof source === "string") return { sessionId: null, sessionRef: null, sessionPath: source };
    const direct = readSessionFields(source);
    if (direct) return direct;
    const nested = readSessionFields(source.session || source.sessionTarget || source.sessionMeta || source.attribution || source.actor);
    if (nested) return nested;
  }
  return { sessionId: null, sessionRef: null, sessionPath: null };
}

// Derive the identity key from an already-normalized { sessionId, sessionRef,
// sessionPath } target, skipping the normalizeSessionTarget() re-scan. Used on
// hot event paths (observer.js) that already hold a normalized target, where
// calling sessionIdentityKey() would otherwise re-run normalizeSessionTarget
// on every single event.
export function sessionKeyFromNormalized(normalized = {}) {
  if (normalized.sessionId) return `sid:${normalized.sessionId}`;
  if (normalized.sessionRef) return `sref:${stableKey(typeof normalized.sessionRef === "string" ? normalized.sessionRef : JSON.stringify(normalized.sessionRef))}`;
  return normalized.sessionPath || "unknown";
}

export function sessionIdentityKey(target = {}) {
  return sessionKeyFromNormalized(normalizeSessionTarget(target));
}

export function sessionTargetDisplay(target = {}) {
...
// ── Usage helpers (merged from usage.js) ──
...
export function usageDedupKey(entry = {}, summaryEntry = {}) {
  ...
  const payload = {
    ...
    sessionId: summaryEntry.sessionId,
    sessionRef: summaryEntry.sessionRef,
  ...
  return `usage:${crypto.createHash("sha256").update(JSON.stringify(payload)).digest("hex").slice(0, 16)}`;
}
...
// Reconcile one in-memory pattern (`stored`) with the version of it written to
// disk by a separate writer (control.js). Returns true when `stored` was
// mutated. The plugin's in-memory detector is authoritative and re-persists its
// whole set, so anything a tool writes to patterns.json that this does NOT
// absorb is silently clobbered on the next flush.
//
// Absorbs:
//   - a strictly newer advisor-distilled `fix` (run_model_advisor in control.js),
//     except on user-approved patterns whose text must not be overwritten;
//   - a manual approve/reject status (+ reviewedAt);
//   - a durable promotion (upgrade only, never downgrade);
//   - a manual approval that outranks a prior machine approval (clears
//     autoApproved so pruneMemory treats it as user-blessed/immortal).
export function absorbDiskPatternState(stored, disk) {
  if (!stored || !disk) return false;
  let changed = false;

  if (disk.advisorUpdatedAt && disk.fix && stored.status !== "approved") {
    const diskTs = Date.parse(disk.advisorUpdatedAt) || 0;
    const memTs = Date.parse(stored.advisorUpdatedAt || "") || 0;
    if (diskTs > memTs && disk.fix !== stored.fix) {
      stored.fix = disk.fix;
      stored.advisorUpdatedAt = disk.advisorUpdatedAt;
      changed = true;
    }
  }

  if (disk.status === "approved" || disk.status === "rejected") {
    if (stored.status !== disk.status) {
      stored.status = disk.status;
      if (disk.reviewedAt) stored.reviewedAt = disk.reviewedAt;
      changed = true;
    }
    // Only promote to durable on approval, never on rejection.
    if (disk.status === "approved" && disk.knowledgeTier === "durable" && stored.knowledgeTier !== "durable") {
      stored.knowledgeTier = "durable";
      changed = true;
    }
    if (disk.status === "approved" && !disk.autoApproved && stored.autoApproved) {
