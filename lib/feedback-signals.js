...
// Platform-independent: `path` is win32 on Windows and posix on Linux, so the
// host's `path.isAbsolute` alone would miss a Windows path on Linux (and vice
// versa) and leak it in full. Normalize separators first, then recognize POSIX,
// drive-letter, and UNC absolute forms on every platform.
function sanitizeRef(ref) {
  const raw = String(ref || "").trim();
  if (!raw) return "";
  const normalized = raw.replace(/\\/g, "/");
  const isAbsolute = path.posix.isAbsolute(normalized) // /foo and //unc/share
    || /^[a-zA-Z]:\//.test(normalized);                // C:/foo (was C:\foo)
  const safe = isAbsolute ? path.posix.basename(normalized) : normalized;
