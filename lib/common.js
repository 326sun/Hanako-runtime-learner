...
export function memoryStrength(pattern, config) {
  ...
  // λ adapts to repetition: frequent patterns forget slower
...
// Shared skill builder — used by both index.js and control.js
export function buildSkillMdFromPatterns(patterns, config, { turnCount = 0, dataDir = "" } = {}) {
  ...
  const prefs = decorated.filter(p => p.type === "preference" && p.injectable).slice(0, 3);
  ...
  if (prefs.length) {
    ...
    if (decorated.filter(p => p.type === "preference" && p.injectable).length > 3) lines.push("- ... more via self_learning_search");
    lines.push("");
  }
