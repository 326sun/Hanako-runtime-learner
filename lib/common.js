...
// returns the pre-rounding float so callers doing incremental subtraction
// (buildSkillMdFromPatterns) stay precise; `estimateTokens` rounds up. Shared by
// buildSkillMdFromPatterns, validation-gate, and doctor — single source of truth
// for the Unicode ranges so the two used to drift apart.
...
export function buildSkillMdFromPatterns(patterns, config, { turnCount = 0, dataDir = "" } = {}) {
  ...
  // CJK-aware token estimation via the shared estimateTokensRaw() (pre-rounding
  // float) so the per-entry trim accounting stays precise and there is only one
  // copy of the Unicode-range table.
