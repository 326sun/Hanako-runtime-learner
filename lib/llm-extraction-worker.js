...
export async function runExtractionTick(ctx, { config = {}, dataDir, now = new Date().toISOString() } = {}) {
  ...
  try {
    ...
    // P9.C: record batch size and duration alongside lastRunAt so extraction
    // throughput/latency is observable without re-deriving it from the queue.
    const durationMs = performance.now() - _t0;
    try { writeJson(extractionStateFile(dataDir), { lastRunAt: now, durationMs, processed, proposalsCreated, failedCount, discardedCount, maxJobs }); } catch {}
    return { ok: true, processed, proposalsCreated, durationMs };
  } catch (err) {
    const durationMs = performance.now() - _t0;
    ctx?.log?.debug?.(`runtime-learner: llm extraction tick error: ${err?.message || err}`);
    return { ok: false, processed, proposalsCreated, durationMs, error: String(err?.message || err) };
  } finally {
    _inFlight = false;
  }
}
