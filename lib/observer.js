...
// ── Inlined from observer-adoption.js ──

function processAdoptionCheck({ sessionPath, runtimeState, detector, tools, persistPatterns, refreshSkill, ctx }) {
  const pending = runtimeState.pendingAdoptionChecks.get(sessionPath);
  if (!pending) return;
  ...
  if (pending.remaining <= 0) {
    ...
    runtimeState.pendingAdoptionChecks.delete(sessionPath);
  }
}
...
export function createObserver(deps) {
  ...
  function flushTurn(sessionPath, event = {}) {
    ...
    for (const errMsg of turn.errors) {
      errorEntries.push({
        date, taskId, sessionPath: key, taskType: exp.taskType, scope,
    ...
    try {
      const batch = {};
      batch[paths.TURNS_FILE] = [{ date, sessionPath: key, tools, errors: turn.errors, stopReason, correction }];
      batch[paths.EXPERIENCE_LOG] = [exp];
      if (paths.EPISODES_FILE) {
        batch[paths.EPISODES_FILE] = [{
          id: taskId, date, sessionPath: key, scope, tools, taskType,
    ...
    try {
      unsubs.push(ctxBus.subscribe((event, sessionPath) => {
        ...
        if (event.type === "tool_execution_end") {
          ...
          handleToolEnd(event, sessionPath);
          return;
        }
