...
export function createObserver(deps) {
  const {
    ...
    paths,
    MAX_SESSIONS,
    // P6.D: shared across every flush in this session so back-to-back turns
    // coalesce into a single in-flight auto-action run and the per-session
    // action budget actually accumulates instead of resetting every flush.
    actionPipelineState = null,
    actionBudgetState = null,
  } = deps;
  ...
  function reloadConfigIfStale() {
    try {
      ...
      if (mtime !== _configMtime) {
        ...
        const nextConfig = { ...liveConfig, ...readJson(paths.CONFIG_FILE, {}) };
        for (const key of Object.keys(liveConfig)) delete liveConfig[key];
        Object.assign(liveConfig, nextConfig);
        detector.setConfig(liveConfig);
      }
    } catch {
  ...
  // sessionTarget is expected to already be normalized ({ sessionId,
  // sessionRef, sessionPath }) by the caller. `key`, if provided, must be the
  // identity key already derived from that same target — passing it avoids
  // re-deriving it (and re-normalizing sessionTarget) on every event.
  function getTurn(sessionTarget, key = null) {
    const target = sessionTarget;
    key = key || sessionKeyFromNormalized(target);
  ...
  function subscribe(ctxBus, config) {
    ...
    try {
      unsubs.push(ctxBus.subscribe((event, sessionMeta) => {
        ...
        const sessionKey = sessionKeyFromNormalized(sessionTarget);
        runtimeState.sessionTargets?.set(sessionKey, sessionTarget);
        const turn = getTurn(sessionTarget, sessionKey);
