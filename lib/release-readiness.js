...
import { readZipEntryNames, verifyDistStructure, verifyZipRoot } from "./dist-verify.js";
...
const TEST_EVIDENCE_PATH = "benchmark-results/test-results.json";
...
function declaredReadmeTestCount(projectRoot) {
  const readme = readText(path.join(projectRoot, "README.md")) || "";
  const badgeMatch = readme.match(/badge\/tests-(\d+)%2F(\d+)-success/);
  return badgeMatch && badgeMatch[1] === badgeMatch[2] ? Number(badgeMatch[1]) : NaN;
}
...
function checkTestEvidence(projectRoot) {
  const evidence = readJson(path.join(projectRoot, TEST_EVIDENCE_PATH));
  if (!evidence) return makeCheck("tests.current_source_passed", false, `missing or invalid ${TEST_EVIDENCE_PATH}; run npm test`);
  const currentFingerprint = computeSourceFingerprint(projectRoot);
  const countsValid = Number.isInteger(evidence.tests)
    && Number.isInteger(evidence.pass)
    && Number.isInteger(evidence.fail)
    && Number.isInteger(evidence.skipped)
    && evidence.tests === evidence.pass + evidence.fail + evidence.skipped;
  const ok = evidence.schemaVersion === 1
    && evidence.exitCode === 0
    && evidence.fail === 0
    && countsValid
    && evidence.sourceFingerprint === currentFingerprint;
  const message = ok
    ? `current source test evidence passed: ${evidence.tests} tests, ${evidence.pass} passed, ${evidence.skipped} skipped`
    : evidence.sourceFingerprint !== currentFingerprint
      ? "test evidence is stale for the current source; rerun npm test"
      : `test evidence is not green: tests=${evidence.tests ?? "unknown"}, fail=${evidence.fail ?? "unknown"}, exit=${evidence.exitCode ?? "unknown"}`;
  return makeCheck("tests.current_source_passed", ok, message, {
...
function checkDistPackage(projectRoot) {
  ...
  let zipRoot = { ok: false, problems: ["missing release/hanako-runtime-learner-dist.zip"] };
  if (zip && zip.size > 0) {
    try {
      zipRoot = verifyZipRoot(readZipEntryNames(fs.readFileSync(zipPath)));
    } catch (err) {
      zipRoot = { ok: false, problems: [`zip root check failed: ${err.message}`] };
    }
  }
  let actualDigest = null;
  let expectedDigest = null;
  try { actualDigest = crypto.createHash("sha256").update(fs.readFileSync(zipPath)).digest("hex"); } catch {}
  try { expectedDigest = fs.readFileSync(digestPath, "utf-8").trim().split(/\s+/)[0]?.toLowerCase() || null; } catch {}
  const digestMatches = !!actualDigest && expectedDigest === actualDigest;
  const ok = dist.ok && !!zip && zip.size > 0 && zipRoot.ok && digestMatches;
  return makeCheck(
    "dist.package_verified",
    ok,
    ok ? `dist package verified with SHA-256 ${actualDigest}` : `dist package verification failed: ${[...dist.problems, ...(zip ? zipRoot.problems : ["missing release/hanako-runtime-learner-dist.zip"]), ...(!digestMatches ? ["ZIP SHA-256 sidecar missing or mismatched"] : [])].filter(Boolean).join("; ")}`,
    { distDir: "dist", zipPath: "release/hanako-runtime-learner-dist.zip", digestPath: "release/hanako-runtime-learner-dist.zip.sha256", actualDigest, expectedDigest, problems: [...dist.problems, ...zipRoot.problems], zipBytes: zip?.size || 0 },
  );
}
...
export function buildReleaseReadiness(projectRoot = process.cwd(), options = {}) {
  ...
  const testEvidence = readJson(path.join(root, TEST_EVIDENCE_PATH));
  const expectedTestCount = Number(options.expectedTestCount
    ?? (options.requireTestEvidence === true ? testEvidence?.tests : declaredReadmeTestCount(root)));

  // P10.B: each check is a thunk so we can time it individually — some (the
  // benchmark corpus load, the complexity scan) are non-trivial and per-check
  // duration helps localize a slow `release:check` run without guessing.
  const checkDefs = [
    () => makeCheck(
    ...
    ...(options.requireTestEvidence === true ? [() => checkTestEvidence(root)] : []),
    () => checkReadmeTestBadge(root, expectedTestCount),
...
export function formatReleaseReadinessReport(result = {}) {
  ...
  lines.push("## Checks");
  lines.push("");
  lines.push("| Check | Status | Duration (ms) | Message |");
  lines.push("|---|---|---:|---|");
  for (const check of result.checks || []) {
    const duration = Number.isFinite(check.durationMs) ? check.durationMs.toFixed(1) : "n/a";
    lines.push(`| ${check.id} | ${check.status} | ${duration} | ${String(check.message || "").replaceAll("|", "\\|")} |`);
  }
  lines.push("");
