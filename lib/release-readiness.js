...
const LTS_VERSION_RE = /^\d+\.\d+\.\d+(?:-lts)?$/;
...
function checkRequiredDocs(projectRoot, docs = REQUIRED_LTS_DOCS) {
  ...
  return makeCheck(
    "docs.required_lts_docs",
    missing.length === 0,
    missing.length === 0 ? "all required LTS docs exist and are non-empty" : `missing or empty docs: ${missing.map((item) => item.path).join(", ")}`,
...
function checkReadmeTestCount(projectRoot, expectedCount) {
  ...
  const textMatch = readme.match(/(\d+)\s*项测试/);
...
export function buildReleaseReadiness(projectRoot = process.cwd(), options = {}) {
  ...
  const checks = [
    makeCheck(
      "package.version_lts_format",
      typeof version === "string" && LTS_VERSION_RE.test(version),
      LTS_VERSION_RE.test(version) ? `package version is release-formatted: ${version}` : `package version is not release-formatted: ${version}`,
      { version },
    ),
    makeCheck(
    ...
    checkRequiredDocs(root, options.requiredDocs || REQUIRED_LTS_DOCS),
    makeCheck(
    ...
    makeCheck(
      ...
      /api freeze|frozen api|冻结/i.test(apiFreeze) && apiFreeze.includes("v4.0"),
      "API freeze document exists and declares the v4.0 freeze surface",
    ...
    checkReadmeTestBadge(root, options.expectedTestCount ?? 496),
    checkReadmeCloneBranch(root, version),
    checkManifestVersion(root, version),
    checkApiFreezeVersion(root, version),
    checkReadmeTestCount(root, options.expectedTestCount ?? 496),
  ];
  ...
  const summary = {
    ...
    nextAction: failed.length === 0 ? "release can proceed after npm run check, npm test, and npm run benchmark pass" : "fix failed release-readiness checks before publishing",
  };
