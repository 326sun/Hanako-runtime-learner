...
const DEFAULT_EXPECTED_TEST_COUNT = 943;
...
function checkDistPackage(projectRoot) {
  ...
  const ok = dist.ok && !!zip && zip.size > 0 && zipRoot.ok;
  return makeCheck(
    "dist.package_verified",
    ok,
    ok ? "dist package and release zip are present and structurally verified" : `dist package verification failed: ${[...dist.problems, ...(zip ? zipRoot.problems : ["missing release/hanako-runtime-learner-dist.zip"])].filter(Boolean).join("; ")}`,
    { distDir: "dist", zipPath: "release/hanako-runtime-learner-dist.zip", problems: [...dist.problems, ...zipRoot.problems], zipBytes: zip?.size || 0 },
  );
}
...
export function buildReleaseReadiness(projectRoot = process.cwd(), options = {}) {
  ...
  const checkDefs = [
    ...
    () => checkReadmeTestBadge(root, options.expectedTestCount ?? DEFAULT_EXPECTED_TEST_COUNT),
    ...
    () => checkReadmeTestCount(root, options.expectedTestCount ?? DEFAULT_EXPECTED_TEST_COUNT),
