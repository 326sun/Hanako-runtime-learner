...
export function applyProposalSafely(learnerDir, id, { configPath = null, requireReview = false, allowedSkillRoots = [] } = {}) {
  ...
  if (proposal.status === "applied") {
    // A crash can land after the proposal record is finalized but before its
    // review is updated. Repeating apply is the reconciliation path.
    try { markReviewForProposal(learnerDir, id, "applied", { recovery: "proposal_already_applied" }); } catch {}
    return proposal;
  }
  ...
  // Match the generic proposal engine's write-ahead recovery contract. If the
  // target already contains the reviewed bytes, a prior run crossed the file
  // boundary and only governance finalization remains.
  if (proposal.status === "applying") {
    const current = safeReadFile(allowed.full, allowed.root, { filesystem: { deny: [] } });
    if (current.ok && current.content === proposal.patch.content) {
      const recoveredAt = new Date().toISOString();
      const recovered = {
        ...proposal,
        status: "applied",
        appliedAt: proposal.appliedAt || recoveredAt,
        updatedAt: recoveredAt,
        result: { ok: true, backupPath: proposal.result?.backupPath || null },
      };
      writeJson(proposalPath(learnerDir, id), recovered);
      try { markReviewForProposal(learnerDir, id, "applied", { recovery: "target_already_applied" }); } catch {}
      try {
        appendEvent(learnerDir, {
          type: "proposal.recovered_applied",
          entityType: "proposal",
          entityId: id,
          summary: `Recovered applied proposal: ${id}`,
        });
      } catch {}
  ...
  let backupPath = proposal.result?.backupPath || null;
  if (!backupPath && fs.existsSync(allowed.full)) {
  ...
  // Persist intent before replacing the governed target. If the process dies
  // after the following write, the next invocation can converge using the
  // exact target bytes and the preserved backup path above.
  const applying = {
    ...proposal,
    status: "applying",
    applicationStartedAt: proposal.applicationStartedAt || new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    result: { ok: false, backupPath },
  };
  writeJson(proposalPath(learnerDir, id), applying);
