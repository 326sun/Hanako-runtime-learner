...
// P8.B: buildAuditDashboard() walks several governance subsystems (agent
// tasks, audit traces, transfer registry, skill promotion state) on every
// call — an explicit, heavy tool a caller may invoke repeatedly (e.g.
// polling) without any of that state having changed in between. Reuse the
// most recently exported dashboard.json unless the caller opts into a fresh
// rebuild via `regenerate: true`, or none has been exported yet.
export function findLatestExportedDashboard(learnerDir) {
  const root = path.join(learnerDir, "audit-dashboard");
  const jsonFiles = walkFiles(root, (file) => path.basename(file) === "dashboard.json");
  const latestJson = latestFile(jsonFiles);
  if (!latestJson) return null;
  const payload = readJson(latestJson, null);
  if (!payload) return null;
  const dir = path.dirname(latestJson);
  ...
  return {
    ...
    safetyPosture: payload.safetyPosture,
