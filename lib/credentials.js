...
function deriveKey() {
  ...
  return crypto.pbkdf2Sync(seed, SALT, ITERATIONS, KEY_LEN, "sha256");
}
...
export function saveCredentials(entries = {}) {
  ...
  if (Object.keys(store).length === 0) {
    try { fs.rmSync(credentialsFile(), { force: true }); } catch {}
    return;
  }
  atomicWriteFileSync(credentialsFile(), JSON.stringify(store, null, 2), "utf-8");
}
 ...
 */
export function mergeCredentials(config = {}) {
 ...
 */
export function extractAndSaveCredentials(patch = {}) {
  const toEncrypt = {};
  for (const key of SENSITIVE_KEYS) {
  ...
  for (const key of SENSITIVE_KEYS) {
    if (key in sanitised) {
      sanitised[key] = "(stored in credentials.enc)";
    }
  }
  return sanitised;
}
...
export function detectPlaintextCredentials(config = {}) {
  ...
  for (const key of SENSITIVE_KEYS) {
    ...
    if (typeof val === "string" && val && val !== "(stored in credentials.enc)" && !val.startsWith("***")) {
      plain.push(key);
    }
  }
  return plain;
