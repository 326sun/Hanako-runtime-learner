...
function stableCandidateValue(value) {
  if (Array.isArray(value)) return value.map(stableCandidateValue);
  if (!value || typeof value !== "object") return value;
  return Object.fromEntries(Object.keys(value).sort().map((key) => [key, stableCandidateValue(value[key])]));
}

export function transferCandidateContentHash(candidate = {}) {
  return crypto.createHash("sha256").update(JSON.stringify(stableCandidateValue(candidate || {}))).digest("hex");
}
...
function transferCandidatePath(baseDir, candidateId) {
  return entityFilePath(transferRegistryDir(baseDir), candidateId, { fallback: "transfer_candidate" });
}
...
function createTransferRegistryRecord(candidate = {}, options = {}) {
  ...
  return {
    schemaVersion: 1,
    id: candidate.id,
    candidateContentHash: transferCandidateContentHash(candidate),
    status,
...
function saveTransferCandidateRecord(baseDir, record = {}) {
  ...
  cleanupMatchingLegacyEntityFile(transferRegistryDir(baseDir), next.id, { fallback: "transfer_candidate" });
...
export function registerTransferCandidate(baseDir, candidate = {}, options = {}) {
  const existing = candidate?.id ? loadTransferCandidateRecord(baseDir, candidate.id) : null;
  if (existing) {
    const incomingHash = transferCandidateContentHash(candidate);
    const existingHash = existing.candidateContentHash || transferCandidateContentHash(existing.candidate);
    if (incomingHash !== existingHash) {
      throw new Error(`transfer candidate id conflict: ${candidate.id}`);
    }
    return { ok: existing.status !== TRANSFER_STATUSES.REJECTED, record: existing, saved: null, idempotent: true };
  }
...
export function loadTransferCandidateRecord(baseDir, candidateId) {
  try {
    const file = resolveEntityFilePath(transferRegistryDir(baseDir), candidateId, { fallback: "transfer_candidate" });
    const record = JSON.parse(fs.readFileSync(file, "utf-8"));
    return record?.id === candidateId ? record : null;
  } catch {
    return null;
  }
}
...
function promotionReadiness(record) {
  ...
  const hasTargetEvidence = (record.validationHistory || []).some((event) => event.status === "passed" && event.evidence?.length > 0);
...
export function recordTransferValidation(baseDir, candidateId, result = {}) {
  ...
  if (status === "passed" && (!Array.isArray(result.evidence) || result.evidence.length === 0)) {
    throw new Error("passed transfer validation requires concrete evidence");
  }
