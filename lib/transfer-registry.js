...
export function safeTransferCandidateId(id) {
  return safeFileSlug(id, "transfer_candidate");
}

export function transferRegistryDir(baseDir) {
  return path.join(baseDir, "cross_project_transfers");
}

export function transferCandidatePath(baseDir, candidateId) {
...
export function createTransferRegistryRecord(candidate = {}, options = {}) {
...
export function saveTransferCandidateRecord(baseDir, record = {}) {
...
export function registerTransferBatch(baseDir, candidates = [], options = {}) {
  const records = [];
  const errors = [];
  for (const candidate of candidates || []) {
    try {
      records.push(registerTransferCandidate(baseDir, candidate, options).record);
    } catch (error) {
      errors.push({ candidateId: candidate?.id || null, error: error.message });
    }
  }
  return {
    ok: errors.length === 0,
    ...
    records,
    summaries: records.map(summarizeTransferCandidate),
    errors,
  };
}
