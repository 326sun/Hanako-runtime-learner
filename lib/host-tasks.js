...
const shuttingDownPlugins = new Set();

function sleep(ms) { return new Promise((resolve) => setTimeout(resolve, ms)); }

/**
 * Best-effort shutdown boundary. The host may not be able to terminate an
 * already-running external operation, so this first prevents new completion
 * effects, asks the host to cancel known jobs, then waits only up to the
 * caller's deadline. A timeout is reported rather than misrepresented as a
 * physical stop guarantee.
 */
export async function shutdownHostTasks(pluginId = DEFAULT_PLUGIN_ID, { timeoutMs = 2_000 } = {}) {
  shuttingDownPlugins.add(pluginId);
  const active = [...inFlight.values()].filter((entry) => entry.pluginId === pluginId);
  await Promise.all(active.map((entry) => cancelTask(entry.ctx, {
    pluginId, taskId: entry.taskId, type: entry.type, reason: "plugin_unloading",
  }).catch(() => {})));
  const deadline = Date.now() + Math.max(0, timeoutMs);
  while (Date.now() < deadline && [...inFlight.values()].some((entry) => entry.pluginId === pluginId)) {
    await sleep(20);
  }
  const remaining = [...inFlight.values()].filter((entry) => entry.pluginId === pluginId);
  return { cancelled: active.length, drained: remaining.length === 0, remaining: remaining.map((entry) => entry.taskId) };
}
...
export async function runHostTask(ctx, {
  ...
  if (shuttingDownPlugins.has(pluginId)) return { ok: false, skipped: "shutting_down", taskId, type };
  if (inFlight.has(key)) return { ok: false, skipped: "in_flight", taskId, type };
  inFlight.set(key, { key, pluginId, taskId, type, ctx });
  try {
    ...
    if (shuttingDownPlugins.has(pluginId) || result?.cancelled || result?.skipped === "cancelled") {
      await cancelTask(ctx, { pluginId, taskId, type, result });
      audit(dataDir, {
        type: "background_task.cancelled",
        entityId: taskId,
        summary: `Cancelled background task: ${type}`,
        data: { taskId, type, result, reason: shuttingDownPlugins.has(pluginId) ? "plugin_unloading" : "cancelled" },
      });
...
export async function setupBackgroundTasks({
  ...
  if (config.backgroundTasksEnabled === false) {
    return { ok: false, skipped: "disabled", useLegacyPath: true, shutdown: () => shutdownHostTasks(pluginId) };
  }
  shuttingDownPlugins.delete(pluginId);
  ...
  if (!support.ok) {
    ...
    return { ok: false, skipped: "unavailable", missing: support.missing, useLegacyPath: true, shutdown: () => shutdownHostTasks(pluginId) };
  }
  ...
  for (const job of jobs) {
    ...
    const registered = await registerTaskHandler(ctx, { pluginId, type: job.type, handlerId: job.type, handler });
    if (!registered.ok) {
      failures.push({ type: job.type, operation: "register-handler", reason: registered.error || registered.skipped || "rejected" });
      continue;
    }
    ...
    if (!scheduled.ok) {
      failures.push({ type: job.type, operation: "schedule", reason: scheduled.error || scheduled.skipped || "rejected" });
    }
  ...
  if (failures.length > 0) {
    audit(dataDir, {
      type: "background_tasks_setup_failed",
      entityId: pluginId,
      summary: "Host task setup was incomplete; retaining legacy opportunistic background path",
      data: { failures },
    });
    return { ok: false, jobs: jobs.length, failures, useLegacyPath: true, shutdown: () => shutdownHostTasks(pluginId) };
  }

  const recovered = await failRecoveringTasks(ctx, { pluginId, dataDir });
  return { ok: true, jobs: jobs.length, recovered, useLegacyPath: false, shutdown: () => shutdownHostTasks(pluginId) };
}
