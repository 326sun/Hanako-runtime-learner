...
export function previewProposalDiff(proposal, { configPath = null } = {}) {
  ...
  if (proposal.type === "pattern_candidate") {
    ...
    if (proposal.generalization) lines.push(`when: ${proposal.generalization}`);
    if (Array.isArray(proposal.evidenceIds) && proposal.evidenceIds.length) lines.push(`evidence: ${proposal.evidenceIds.join(", ")}`);
    return {
      ok: true,
      proposalId: proposal.id,
      type: proposal.type,
