...
import { normalizeSessionTarget, sessionIdentityKey, sessionTargetDisplay } from "./helpers.js";
...
export function inspectSessionIdentityCoverage(file, { maxLines = 5000 } = {}) {
  ...
  for (const line of readJsonlTailLines(file, { maxLines })) {
    try {
      const row = normalizeLogSessionRow(JSON.parse(line));
      summary.total += 1;
      if (row.sessionId || row.sessionRef) summary.withStableIdentity += 1;
      else if (row.sessionPath) summary.legacyPathOnly += 1;
      else summary.unknown += 1;
    } catch {}
  }
  summary.coverageRatio = summary.total > 0 ? summary.withStableIdentity / summary.total : 1;
  return summary;
}
