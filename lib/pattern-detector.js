...
import { toolCategory, normalizeToolName, preferencePatternId } from "./helpers.js";
import {
  knowledgeTier,
  memoryStrength,
  decayedScore,
...
import { MAX_PATTERN_COUNT } from "./pattern-detector-utils.js";
...
export class PatternDetector {
  ...
  pruneMemory() {
    ...
    if (durableEntries.length > durableMax) {
      ...
      for (const [id] of durableEntries.slice(durableMax)) {
    ...
    const survivors = [];
    for (const [id, p, strength] of evaluable) {
      if (decayedScore(p, this.config) < 1) {
        this._forgetPattern(id);
        pruned += 1;
      } else {
        survivors.push([id, p, strength]);
      }
    }
    ...
    if (survivors.length > MAX_PATTERN_COUNT * 2) {
      ...
      for (let i = limit; i < survivors.length; i++) {
        this._forgetPattern(survivors[i][0]);
        pruned += 1;
      }
    }
