...
export class PatternDetector {
  ...
  ingest(exp) {
    ...
    // Also skip sequences where every tool category is "其他" (unknown tools —
    // these produce noisy, meaningless workflow patterns).
    if (exp.toolsUsed.length >= 2) {
      const cats = exp.toolsUsed.map(t => toolCategory(normalizeToolName(t)));
      const uniqueCats = [...new Set(cats)].sort();
      if (uniqueCats.length >= 2 && !uniqueCats.every((c) => c === "其他")) {
  ...
  /**
   * Prune the pattern store. Three stages: (1) cap durable patterns by count,
   * (2) score-floor prune decayed patterns, (3) strength-based cap for
   * remaining non-durable/non-manually-approved patterns. Clears seqCache
   * and orphan relations for evicted patterns.
   *
   * @returns {number} count of pruned patterns
   */
  pruneMemory() {
    let pruned = 0;
  ...
  all() {
    ...
    const filtered = [...this.patterns.values()].filter(pattern => {
    ...
    // Decoration (knowledgeTier/status/decayedScore/injectable + sort) is shared
    // with decoratePatterns() so the two never drift.
    this._cachedAll = decoratePatterns(filtered, this.config);
