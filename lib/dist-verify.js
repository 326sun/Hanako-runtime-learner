...
export function readZipEntryNames(buffer) {
  ...
  for (let i = 0; i < count; i += 1) {
    if (offset < 0 || offset + 46 > buf.length) break;
    ...
    names.push(buf.toString("utf8", nameStart, nameEnd));
 ...
 */
export function verifyZipRoot(entryNames = [], { requiredFiles = REQUIRED_DIST_FILES, requiredToolFiles = REQUIRED_TOOL_FILES } = {}) {
  const problems = [];
  const names = entryNames.map((n) => String(n).replace(/\\/g, "/"));
  for (const required of requiredFiles) {
    if (!names.includes(required)) problems.push(`zip must contain required runtime file ${required}`);
  }
