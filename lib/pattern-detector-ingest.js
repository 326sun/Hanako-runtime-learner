import { preferencePatternId, safeText, stableKey } from "./helpers.js";
...
function usageEvidenceQuote(entry = {}, totalTokens = 0, date = null) {
  const operation = entry.operation || entry.subsystem || "unknown";
  const parts = [
    `at=${date || entry.date || "unknown"}`,
  ...
  if (Number.isFinite(Number(entry.inputTokens))) parts.push(`inputTokens=${Number(entry.inputTokens)}`);
  if (Number.isFinite(Number(entry.outputTokens))) parts.push(`outputTokens=${Number(entry.outputTokens)}`);
  if (entry.error) parts.push(`error=${safeText(entry.error, 120)}`);
  return parts.join(" ");
}

function makeUsageEvidence(entry = {}, totalTokens = 0, date = null) {
  return makeEvidence({
    type: "usage",
    file: "usage_summary.json",
    date,
    id: entry.requestId || entry.id || null,
    quote: usageEvidenceQuote(entry, totalTokens, date),
  });
}
...
export function ingestUsage(detector, entry = {}) {
  ...
  for (const pattern of patterns) {
    const evidence = makeUsageEvidence(entry, totalTokens, now);
    ...
    if (existing) {
      ...
      attachEvidence(existing, evidence);
