...
export function runPostFlushPipeline({
  ...
  maybeRunExtraction = () => Promise.resolve(),
  reason,
  sessionHandle = null,
  before = [],
  after = [],
  ...
  workspaceRoot = process.cwd(),
  skipPrune = false,
  // P6.D: shared, caller-owned state so back-to-back flushes (e.g. several
  // turns completing before the previous auto-action pipeline run resolves)
  // don't stack up N concurrent fire-and-forget runAutoActionPipeline calls,
  // and so the per-session action budget actually accumulates across flushes
  // instead of resetting every call. Both default to null/fresh-per-call to
  // preserve prior behavior for callers that don't opt in.
  actionPipelineState = null,
  budgetState = null,
}) {
  for (const hook of before) {
    try { hook(); } catch (err) { ctx?.log?.debug?.(`runtime-learner: before-hook ${err.message}`); }
  }
  // before-hooks may refresh the shared live config object from disk. Resolve
  // policy only after those hooks so the current turn cannot execute once with
  // stale auto-action or governance settings.
  ...
  if (!actionPipelineState || !actionPipelineState.inFlight) {
    if (actionPipelineState) actionPipelineState.inFlight = true;
    runAutoActionPipeline({ learnerDir, config: effectiveConfig, patterns: allPatterns, usage, errors, input, workspaceRoot, budgetState: budgetState || createBudgetState(), ctx })
      .catch((err) => ctx?.log?.warn?.(`runtime-learner: auto-action pipeline failed: ${err.message}`))
      .finally(() => { if (actionPipelineState) actionPipelineState.inFlight = false; });
  }
  ...
  // v5.0 M2: synchronous candidate enqueue + async LLM extraction tick, gated by
  // llmExtractionEnabled (default false → fully no-op, identical to v4).
  maybeRunExtraction(reason, sessionHandle, allPatterns).catch(
    (err) => ctx?.log?.debug?.(`runtime-learner: llm extraction rejected: ${err?.message || err}`));
