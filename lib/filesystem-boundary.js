...
// Resolve symlinks as far as the path exists on disk, then re-append any
// not-yet-created tail. This way a symlinked ancestor cannot disguise a denied
// target (e.g. workspace/innocent -> workspace/.git) even when the final leaf
// does not exist yet.
function realpathBestEffort(resolved) {
  const base = nearestExistingParent(resolved);
  if (!base) return resolved;
  const baseReal = realpathIfExists(base) || base;
  if (base === resolved) return baseReal;
  const rel = path.relative(base, resolved);
  return rel ? path.join(baseReal, rel) : baseReal;
}
...
export function isPathForbidden(targetPath, policy) {
  // Follow symlinks before matching, and compare case-insensitively — deny
  // protection must hold on case-insensitive filesystems (Windows/macOS) and
  // must not be bypassable via an innocuously named in-workspace symlink.
  const normalized = normalizeSlashes(realpathBestEffort(path.resolve(targetPath))).toLowerCase();
  const denyPatterns = policy?.filesystem?.deny || [];

  for (const pattern of denyPatterns) {
    const cleaned = normalizeSlashes(pattern).toLowerCase();
    if (!cleaned) continue;
