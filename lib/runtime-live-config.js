...
function loadConfig(paths) {
  try {
    ...
    // Permission, I/O and path errors are operational failures, not malformed
    // user configuration.  Falling through here used to silently replace the
    // active configuration with defaults, which could re-enable features the
    // operator had explicitly disabled.  Let initial startup fail safely; the
    // live-refresh caller already catches this and keeps the last good config.
    throw e;
  }
  try { writeJson(paths.CONFIG_FILE, DEFAULT_CONFIG); } catch {}
...
export function bridgePanelConfig(ctx, rt) {
  const credentialOptions = { dataDir: rt.paths.DATA_DIR };
  ...
  try {
    ...
    if (Object.keys(panelCreds).length > 0) {
      saveCredentials({ ...loadCredentials(credentialOptions), ...panelCreds }, credentialOptions);
  ...
  try {
    ...
    if (plaintextKeys.length > 0) {
      ...
      saveCredentials({ ...loadCredentials(credentialOptions), ...toEncrypt }, credentialOptions);
  ...
  rt.config = mergeCredentials(config, credentialOptions);
...
export function wireLiveConfigAndDisposal(ctx, rt, runtimeState) {
  ...
  const refreshConfigFromPanel = () => {
    try {
      ...
      try {
        const panelCreds = panelCredentialsToStore(ctx.config);
        if (Object.keys(panelCreds).length > 0) saveCredentials({ ...loadCredentials(credentialOptions), ...panelCreds }, credentialOptions);
      } catch {}
      applyLiveConfig({ config: rt.config, configRef: rt.configRef, detector: rt.detector }, mergeCredentials(bridged, credentialOptions));
