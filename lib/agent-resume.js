...
export function approveAgentTask(baseDir, taskId, { requestId = null, reason = "approved by user" } = {}) {
  ...
  const request = requestId
    ? (state.approvalRequests || []).find((item) => item.id === requestId)
    : latestPendingApproval(state);
  if (!request || request.status !== "pending") throw new Error("pending approval request not found");
...
export function cancelAgentTask(baseDir, taskId, { requestId = null, reason = "cancelled by user" } = {}) {
  let { state, trace } = loadBundle(baseDir, taskId);
  const pending = latestPendingApproval(state);
  if (requestId || pending) {
    state = resolveApprovalRequest(state, requestId || pending.id, "cancelled", { reason });
  }
  state = transitionAgentState(state, AGENT_STATES.CANCELLED, { node: state.currentNode, reason });
  state.updatedAt = now();
  trace = appendAuditEvent(trace, { type: "human.cancelled", node: state.currentNode, state: state.state, summary: reason, data: { requestId: requestId || pending?.id || null } });
  const saved = saveBundle(baseDir, state, trace);
