...
// this plugin. Hanako v0.345+ exposes this capability; if it is unavailable or
// fails, the official source fails soft instead of scraping host credential
// files or bypassing host-side audit/routing with a direct request.
 ...
 */
export function buildAdvisorStatus({ outcome, prev = null, lastRunAt, reason = null, source = null, suggestionCount = 0, durationMs = null, nextEligibleAt = null }) {
  const priorFailures = Number(prev?.consecutiveFailures) || 0;
  // P9.A: duration/nextEligibleAt are observability-only additions alongside
  // the existing status/reason/consecutiveFailures fields; skippedBecause is
  // an unambiguous alias of `reason` for the skipped outcome specifically, so
  // a caller doesn't need to check `status` first to know whether `reason`
  // means "skipped" or "errored".
  const base = { lastRunAt, durationMs: Number.isFinite(durationMs) ? durationMs : null, nextEligibleAt: nextEligibleAt || null };
  if (outcome === "success") {
    return { ...base, status: "success", reason: null, skippedBecause: null, source: source || "unknown", suggestionCount, consecutiveFailures: 0 };
  }
  if (outcome === "skipped") {
    return { ...base, status: "skipped", reason: reason || "unknown", skippedBecause: reason || "unknown", source: null, suggestionCount: 0, consecutiveFailures: priorFailures };
  }
  return { ...base, status: "error", reason: reason || "unknown", skippedBecause: null, source: null, suggestionCount: 0, consecutiveFailures: priorFailures + 1 };
}
...
function shouldRun(config, { patternIds = [], ctx = null, stateFile = MODEL_ADVICE_STATE_FILE } = {}) {
  ...
  if (Number.isFinite(lastRunMs) && elapsedMs < minMs) {
    return { ok: false, reason: "rate limited", retryAfterMs: minMs - elapsedMs };
  }
...
export function resolveAdvisorConfig(config, ctx = null) {
  ...
  if (source === "official") {
    ...
    return { ok: false, reason: "official model sampling capability unavailable" };
  }
  ...
  return {
    ...
    config: {
      ...config,
      modelAdvisorResolvedSource: "private",
...
async function sampleViaHttp(runtimeConfig, prompt, maxTokens) {
  ...
  // Use direct global fetch for the explicitly configured private endpoint. The
...
async function sampleWithResolvedConfig({ runtimeConfig, config, ctx, prompt, maxTokens }) {
  ...
  try {
    ...
    ctx?.log?.warn?.(`runtime-learner: official model sampling failed (${busErr?.message || "unknown"}); no direct credential fallback was attempted`);
    throw busErr;
  }
}
...
export async function runModelAdvisor({ config, patterns = [], usage = null, capabilities = null, reason = "scheduled", ctx = null, dataDir = null, adviceFile = null, stateFile = null }) {
  ...
  if (!gate.ok) return { ok: false, skipped: true, reason: gate.reason, retryAfterMs: gate.retryAfterMs };
...
export function createAdvisorRunner({
  ...
  let _inFlight = false;
  let _rateLimitSkipUntil = 0;
  let _rateLimitIntervalMinutes = null;

  const rateLimitIntervalMinutes = (config) => Math.max(1, Number(config.modelAdvisorMinIntervalMinutes || 60));

  const isRateLimitCached = (config) =>
    _rateLimitSkipUntil > Date.now()
    && _rateLimitIntervalMinutes === rateLimitIntervalMinutes(config);

  const cacheRateLimit = (config, retryAfterMs) => {
    if (!Number.isFinite(retryAfterMs) || retryAfterMs <= 0) return;
    _rateLimitIntervalMinutes = rateLimitIntervalMinutes(config);
    _rateLimitSkipUntil = Date.now() + retryAfterMs;
  };

  // P9.A: the earliest the advisor could plausibly run again — the precise
  // rate-limit retryAfterMs when available (a "skipped: rate limited"
  // result), otherwise the standard interval measured from this run.
  const nextEligibleAtFromInterval = (config, fromIso, retryAfterMs = null) => {
    if (Number.isFinite(retryAfterMs) && retryAfterMs > 0) return new Date(Date.now() + retryAfterMs).toISOString();
    const intervalMs = rateLimitIntervalMinutes(config) * 60_000;
    const fromMs = Date.parse(fromIso) || Date.now();
  ...
  async function maybeRun(reason, sessionHandle = null, cachedAll = null) {
    ...
    if (isRateLimitCached(config)) {
      if (!_cachedAdvisorSkip("rate limited")) {
        ctx.log.info("runtime-learner: model advisor skipped: rate limited");
      }
      return;
    }
    if (_inFlight) return;
    ...
    try {
      ...
      if (result.ok) {
        ...
        writeJson(advisorStatusFile, buildAdvisorStatus({
          ...
          nextEligibleAt: nextEligibleAtFromInterval(config, lastRunAt),
        }));
        _rateLimitSkipUntil = 0;
        if (count > 0) {
          logActivity({
            type: "model_advisor",
        ...
        writeJson(advisorStatusFile, buildAdvisorStatus({
          ...
          durationMs: performance.now() - _t0,
          nextEligibleAt: nextEligibleAtFromInterval(config, lastRunAt, result.retryAfterMs),
        }));
        if (result.reason === "rate limited") cacheRateLimit(config, result.retryAfterMs);
