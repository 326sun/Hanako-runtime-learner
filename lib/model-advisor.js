...
function shouldRun(config, { patternCount = 0 } = {}) {
  ...
  // Data-driven gate: skip if too few new patterns since last run
  const minDelta = Math.max(0, Number(config.minAdvisorNewPatterns || 3));
  if (minDelta > 0 && Number.isFinite(state.lastPatternCount)) {
    const delta = patternCount - state.lastPatternCount;
    if (delta < minDelta) {
      return { ok: false, reason: `only ${delta} new pattern(s), need ${minDelta}` };
    }
  }
...
export async function runModelAdvisor({ config, patterns = [], usage = null, capabilities = null, reason = "scheduled" }) {
  const gate = shouldRun(config, { patternCount: patterns.length });
  ...
  writeJson(MODEL_ADVICE_STATE_FILE, { lastRunAt: advice.updatedAt, lastPatternCount: patterns.length });
