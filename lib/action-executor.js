...
export async function executeActionPlan(actionPlan = {}, context = {}) {
  ...
  // TODO(P1): workspaceRoot falling back to process.cwd() is a boundary
  // implicitisation risk for write actions.  Callers should always pass an
  // explicit workspaceRoot; when they don't, a warning should be logged via
  // ctx before we eventually fail-closed for write-action paths.
  const workspaceRoot = path.resolve(context.workspaceRoot || process.cwd());
  let result = { actionId: actionPlan.id || null, actionType, status: "failed", logs: [], artifacts: [], errors: [] };
