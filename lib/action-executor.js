...
async function attemptRepairOnce(txn, actionPlan, { workspaceRoot, config, taskScope = null, failedResult = null }) {
  ...
  const approvedPaths = new Set(primaryPatchPaths(actionPlan).map((file) => path.resolve(workspaceRoot, file)));
  const repairPaths = [...fileWrites, ...filePatches].map((item) => item?.path).filter(Boolean);
  const expanded = repairPaths.filter((file) => !approvedPaths.has(path.resolve(workspaceRoot, file)));
  if (expanded.length > 0) {
    return { attempted: true, ok: false, error: `repair scope expands the approved file set: ${expanded.join(", ")}` };
  }
  const repairProposal = {
    id: `${actionPlan.id || "action"}:repair`,
    type: "code_patch",
    patch: { filePatches, fileWrites },
  };
  const repairGate = previewAndGate(repairProposal, { workspaceRoot, taskScope, config });
  if (!repairGate.ok || repairGate.decision !== SCOPE_DECISION.ALLOW) {
    return {
      attempted: true,
      ok: false,
      error: `repair scope gate rejected: ${repairGate.scopeGate?.reason?.join("; ") || repairGate.decision || "unknown"}`,
      scopeGate: repairGate.scopeGate,
      diffPreview: repairGate.diffPreview,
    };
  }
...
function primaryPatchPaths(actionPlan = {}) {
  const plan = actionPlan.plan || {};
  return [...new Set([
    ...normalizeArray(plan.fileWrites).map((item) => item.path),
    ...normalizeArray(plan.filePatches || plan.patches).map((item) => item.path),
  ].filter(Boolean))];
}
...
export async function executeActionPlan(actionPlan = {}, context = {}) {
  ...
  if (!verification.verified && transaction && repairEnabled) {
    ...
    const repair = await attemptRepairOnce(transaction, actionPlan, {
      workspaceRoot,
      config,
      taskScope: context.taskScope || config.taskScope || null,
      failedResult,
