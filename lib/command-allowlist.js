...
function deniedByDangerousRuntimeFlag(command) {
  ...
  const dangerous = new Set(["-e", "--eval", "-p", "--print", "-r", "--require", "--import"]);
...
function deniedByBuiltinPattern(command) {
  ...
  for (const segment of segments) {
    const first = segment.split(/\s+/)[0]?.toLowerCase();
...
function deniedByConfiguredPattern(command, denylist = []) {
  ...
  for (const raw of denylist || []) {
    ...
    // Multi-token deny entries are treated as command-prefix patterns on each
    // shell segment. Single-token entries are matched as shell tokens only.
    if (/\s/.test(denied)) {
      const re = new RegExp(`^${escapeRegExp(denied)}(?:\\s|$)`, "i");
      if (segments.some((segment) => re.test(segment))) return `command matches denylist entry: ${raw}`;
      continue;
    }

    const tokenRe = new RegExp(`(^|\\s)${escapeRegExp(denied)}(?=$|\\s)`, "i");
    if (segments.some((segment) => tokenRe.test(segment))) return `command contains denied token: ${raw}`;
  }
