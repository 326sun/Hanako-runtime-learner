...
export function isNotYetValid(item, now = Date.now()) {
  const validFrom = item?.scope?.validFrom ?? item?.validFrom;
...
export function admitMemory(candidate, queryContext = {}, config = {}) {
  ...
  if (isNotYetValid(candidate)) return { admitted: false, reason: "not yet valid" };
