...
import { decoratePatterns, estimateTokensRaw } from "./scoring.js";
...
export function buildSkillMdFromPatterns(patterns, config, { turnCount = 0, dataDir = "" } = {}) {
  ...
  for (const pattern of decorated) {
    if (!pattern.injectable) continue;
    injectableCount += 1;
    if (pattern.type === "preference" && (
      pattern.knowledgeTier !== "durable" || pattern.status === "approved" || pattern.advisorUpdatedAt
    )) {
      allPrefs.push(pattern);
      if (prefs.length < 5) prefs.push(pattern);
    } else if (pattern.type === "workflow") {
      if (workflows.length < 3) workflows.push(pattern);
    } else if (pattern.type === "error" || pattern.type === "usage") {
      if (risks.length < 3) risks.push(pattern);
    }
  }
  ...
  let lines = [
    ...
    "",
    turnCount
      ? `Observed ${turnCount} turns, ${patterns.length} patterns (${injectableCount} active).`
      : `${patterns.length} patterns, ${injectableCount} active.`,
    "",
    "## How to use",
