...
const HUMAN_INTERRUPT_REASONS = Object.freeze({
...
export function resolveApprovalRequest(agentState = {}, requestId, decision, extra = {}) {
  ...
  next.approvalRequests = (next.approvalRequests || []).map((request) => {
    ...
    if (request.status && request.status !== "pending") {
      throw new Error(`approval request already resolved: ${requestId}`);
    }
    if (request.taskId && next.taskId && request.taskId !== next.taskId) {
      throw new Error(`approval request task mismatch: ${request.taskId} != ${next.taskId}`);
    }
    if (request.node && next.currentNode && request.node !== next.currentNode) {
      throw new Error(`approval request node mismatch: ${request.node} != ${next.currentNode}`);
    }
