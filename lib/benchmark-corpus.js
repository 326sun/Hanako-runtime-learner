...
function loadBenchmarkScenario(filePath) {
  let scenario;
  try {
...
function loadBenchmarkBaseline(filePath) {
  if (!filePath || !fs.existsSync(filePath)) return null;
  const data = readJson(filePath);
  return data.metrics ? data : { metrics: data };
}

function loadBenchmarkThresholds(filePath) {
...
export async function runBenchmarkCorpus(options = {}, context = {}) {
  ...
  if (scenarios.length === 0) {
    const result = {
      ...
      corpus: { root: corpus.root, scenarioCount: corpus.scenarioCount, selectedScenarioCount: 0 },
      runs: [],
      metrics: {
        total: 0,
        task_success_rate: null,
        auto_execution_success_rate: null,
        rollback_success_rate: null,
        repair_success_rate: null,
        false_auto_apply_rate: null,
        manual_escalation_rate: null,
        token_overhead: null,
        latency_overhead: null,
        skill_effectiveness: null,
      },
      regressions: [],
      errors: [options.ids?.length ? "no benchmark scenario matched the requested id(s)" : "benchmark corpus contains no scenarios"],
    };
