...
import { readJson, writeJson, mergeConfig } from "./common.js";

// ── Budget ──────────────────────────────────────────────────────────────────

function autoActionBudget(config = {}) {
...
function countJudgedFeedback(rows = []) {
  let judged = 0, success = 0, failure = 0;
  for (const row of rows) {
    if (row.effective === true) {
      judged += 1;
      success += 1;
    } else if (row.effective === false) {
      judged += 1;
      failure += 1;
    }
  }
  return { judged, success, failure };
}

function groupFeedbackByActionType(rows = [], { limit = 20, onlyType = null } = {}) {
  const groups = new Map();
  for (const row of rows) {
    const type = row?.actionType;
    if (!type || (onlyType && type !== onlyType)) continue;
    let group = groups.get(type);
    if (!group) {
      group = [];
      groups.set(type, group);
    }
    if (limit <= 0 || group.length < limit) group.push(row);
  }
  return groups;
}
...
export function summarizeActionEffectiveness(learnerDir, actionType, { limit = 20 } = {}) {
  ...
  const { judged, success, failure } = countJudgedFeedback(rows);
  return { actionType, total: rows.length, judged, success, failure, successRate: judged ? success / judged : null };
}
...
export function updateActionPolicyWeights(learnerDir, { actionType = null, limit = 20 } = {}) {
  ...
  const allRows = readActionFeedback(learnerDir);
  const grouped = groupFeedbackByActionType(allRows, { limit, onlyType: actionType });
  const actionTypes = actionType ? [actionType] : [...grouped.keys()];
  for (const type of actionTypes) {
    const rows = grouped.get(type) || [];
    const { success, failure } = countJudgedFeedback(rows);
    ...
    let recentFailures = 0, recentJudged = 0;
    for (const row of rows) {
      if (row.effective !== true && row.effective !== false) continue;
      recentJudged += 1;
      if (row.effective === false) recentFailures += 1;
      if (recentJudged >= 3) break;
    }
    if (recentJudged === 3 && recentFailures === 3) autoSuspended = true;
