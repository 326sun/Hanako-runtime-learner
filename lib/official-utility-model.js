...
function parseAddedModelProviders(text) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("-")) {
      // A sequence item before any provider mapping key means `providers:` was
      // written as a list — reject, we don't guess at list-shaped credentials.
      if (!current) {
        return { ok: false, providers: {}, error: "provider credentials must be a mapping, not a sequence" };
      }
      // Otherwise it's a sequence nested under a provider field (e.g. a
      // `models:` list). It carries no credential data, so skip the line rather
      // than discard the credentials already parsed for this provider.
      continue;
    }
    ...
    // Skip any line indented deeper than this provider's direct-field level. It
    // belongs to a nested structure under a non-credential field — a `models:`
    // sequence or a `model_defaults:` mapping keyed by model ids that may contain
    // characters (like "/") no credential key uses. Skipping keeps the parse from
    // aborting and discarding credentials already parsed for this provider.
    if (current && fieldIndent !== -1 && indent > fieldIndent) continue;
    ...
    // The first field under a provider sets its direct-field indent, which the
    // nested-skip above uses to separate credentials from deeper nested content.
