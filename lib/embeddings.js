...
function cacheKey(endpoint, model, text) { return `${shortHash(endpoint)}:${model}:${shortHash(text)}`; }
...
export async function embedTexts(texts, config, { fetchImpl, cache, cacheFile, timeoutMs = 15000 } = {}) {
  ...
  texts.forEach((t, i) => {
    const key = cacheKey(resolved.url, resolved.model, t);
    ...
    try {
      ...
      missIdx.forEach((orig, j) => {
        ...
        if (Array.isArray(vec)) {
          vectors[orig] = vec;
          store[cacheKey(resolved.url, resolved.model, texts[orig])] = { vector: vec, createdAt: Date.now(), lastUsedAt: Date.now() };
          cacheStats.stored += 1;
        }
      });
      cacheStats.fetched = missText.length;
      if (!cache) {
        const targetFile = cacheFile || defaultCacheFile();
        // Another request may have populated different keys while this network
        // call was in flight.  Re-read immediately before the synchronous write
        // and merge, so concurrent requests in this runtime do not erase each
        // other's results.  Our freshly fetched entries win only for their keys.
        const latest = loadEmbedCache(targetFile);
        const saved = saveEmbedCache({ ...latest, ...store }, targetFile, { maxEntries: config.semanticCacheMaxEntries });
